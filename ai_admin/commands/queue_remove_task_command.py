        ...
        **kwargs,
        ...
        try:
            ...
            if task.status == "running" and not force:
                raise CustomError(
                    f"Task {task_id} is currently running. Use force=True to remove it"
                )

            success = await queue_manager.remove_task(task_id, force=force)
            if not success:
            ...
            return {
                ...
                "task_id": task_id,
                "removed_status": task.status,
                "force_removed": force and task.status == "running",
                "removed_at": task.updated_at,
            }
