"""CRL operations command for AI Admin.
        ...
        try:
            if action == "create":
                if not ca_cert_path or not ca_key_path:
                    raise ValueError(
                        "CA certificate and key paths are required for create action"
                    )
                ...
                result = await self.cert_config.update_crl(
                    crl_path=crl_path, revoked_serials=revoked_serials or []
                )
                ...
                if not crl_path or not ca_cert_path:
                    raise ValueError(
                        "CRL and CA certificate paths are required for verify action"
                    )
