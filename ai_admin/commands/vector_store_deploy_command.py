...
class VectorStoreDeployCommand(Command):
    ...
    async def _deploy_vector_store(self, namespace: str, vector_store_image: str, 
                                 ...
                                 storage_class: str, storage_size: str) -> SuccessResult:
        ...
        try:
            ...
            return SuccessResult(data={
                ...
                "access_info": {
                    ...
                    "redis_internal": "localhost:6379"  # Redis доступен внутри пода
                },
    ...
    async def _create_persistent_volume(self, namespace: str, storage_class: str, 
        ...
        try:
            ...
            pv_name = f"faiss-storage-{namespace}"
            
            # Check if PV already exists
            try:
                v1.read_persistent_volume(pv_name)
                return SuccessResult(data={
                    "message": f"Persistent Volume {pv_name} already exists",
                    "name": pv_name
                })
            except ApiException as e:
                if e.status != 404:
            ...
            pv = client.V1PersistentVolume(
                ...
                spec=client.V1PersistentVolumeSpec(
                    ...
                    host_path=client.V1HostPathVolumeSource(
                        path=f"/tmp/faiss-storage-{namespace}"
                    )
                )
            )
            ...
            return SuccessResult(data={
                "message": f"Persistent Volume {pv_name} created successfully",
                "name": pv_name,
            ...
            return ErrorResult(
                message=f"Failed to create Persistent Volume: {str(e)}",
                code="PV_CREATION_FAILED",
                details={"exception": str(e)}
            )
                    ...
                    containers=[
                        ...
                        client.V1Container(
                            ...
                            env=[
                                ...
                                client.V1EnvVar(
                                    name="REDIS_URL",
                                    value="redis://localhost:6379"  # Redis доступен внутри пода
                                )
                            ]
                        ),
    ...
    async def _delete_vector_store(self, namespace: str) -> SuccessResult:
        ...
        try:
            ...
            # Delete pod
            try:
                v1.delete_namespaced_pod("vector-store-pod", namespace)
            ...
            # Delete service
            try:
                v1.delete_namespaced_service("vector-store-service", namespace)
            ...
            # Delete ConfigMap
            try:
                v1.delete_namespaced_config_map("vector-store-config", namespace)
            ...
            # Delete PVC
            try:
                v1.delete_namespaced_persistent_volume_claim("faiss-storage-claim", namespace)
    ...
    async def _get_deployment_status(self, namespace: str) -> SuccessResult:
        ...
        try:
            ...
            # Get pod status
            try:
                pod = v1.read_namespaced_pod("vector-store-pod", namespace)
                pod_status = {
            ...
            # Get service status
            try:
                service = v1.read_namespaced_service("vector-store-service", namespace)
                service_status = {
    ...
    async def _get_logs(self, namespace: str, pod_name: Optional[str] = None, container: Optional[str] = None) -> SuccessResult:
        ...
        try:
            ...
            if container:
                ...
                try:
                    container_logs = v1.read_namespaced_pod_log(target_pod, namespace, container=container)
                ...
                for container_name in ["vector-store", "redis"]:
                    try:
                        container_logs = v1.read_namespaced_pod_log(target_pod, namespace, container=container_name)
