        ...
        **kwargs,
        ...
        try:
            ...
            if task.status in ["completed", "failed", "cancelled"]:
                raise CustomError(
                    f"Task {task_id} is already {task.status} and cannot be cancelled"
                )

            if task.status == "running" and not force:
                raise CustomError(
                    f"Task {task_id} is currently running. Use force=True to cancel it"
                )
