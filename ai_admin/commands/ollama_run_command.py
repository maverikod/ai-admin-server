...
class OllamaRunCommand(Command):
    ...
    async def execute(self, 
        ...
        try:
            ...
            final_model_name = model_name or model
            if not final_model_name:
                return ErrorResult(
                    message="Either 'model' or 'model_name' parameter is required",
                    code="MISSING_MODEL_PARAMETER",
                    details={"provided_params": {"model": model, "model_name": model_name}}
                )
            
            if use_queue:
                return await self._run_model_queued(final_model_name, prompt, max_tokens, temperature)
            else:
                return await self._run_model_direct(final_model_name, prompt, max_tokens, temperature)
    ...
    def get_schema(cls) -> Dict[str, Any]:
        ...
        return {
            ...
            "properties": {
                "model": {
                    "type": "string",
                    "description": "Name of the Ollama model to use (alias for model_name)"
                },
                "model_name": {
                    "type": "string",
