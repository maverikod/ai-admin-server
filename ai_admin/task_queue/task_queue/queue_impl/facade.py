...
from .core import TaskQueueCore
...
class TaskQueue:
    ...
    async def add_task(self, task):
        return self.taskQueueCore.add_task(task)

    async def get_task(self, task_id):
        return self.taskQueueCore.get_task(task_id)

    async def get_all_tasks(self):
        return self.taskQueueCore.get_all_tasks()

    async def get_tasks_by_status(self, status):
        return self.taskQueueCore.get_tasks_by_status(status)

    async def get_tasks_by_type(self, task_type):
        return self.taskQueueCore.get_tasks_by_type(task_type)

    async def get_tasks_by_category(self, category):
        return self.taskQueueCore.get_tasks_by_category(category)

    async def get_tasks_by_tag(self, tag):
        return self.taskQueueCore.get_tasks_by_tag(tag)

    async def cancel_task(self, task_id, force):
        return self.taskQueueCore.cancel_task(task_id, force)

    async def remove_task(self, task_id, force):
        return self.taskQueueCore.remove_task(task_id, force)

    async def pause_task(self, task_id):
        return self.taskQueueCore.pause_task(task_id)

    async def resume_task(self, task_id):
        return self.taskQueueCore.resume_task(task_id)

    async def retry_task(self, task_id):
        return self.taskQueueCore.retry_task(task_id)

    async def get_task_summary(self, task_id):
        return self.taskQueueCore.get_task_summary(task_id)

    async def clear_completed(self):
        return self.taskQueueCore.clear_completed()

    async def get_queue_stats(self):
        return self.taskQueueCore.get_queue_stats()

    async def _try_start_next_task(self):
        return self.taskQueueCore._try_start_next_task()

    async def _execute_task(self, task):
        return self.taskQueueCore._execute_task(task)
    ...
    async def _execute_k8s_deployment_create_task(self, task):
        return self.k8sKindExecutor._execute_k8s_deployment_create_task(task)

    async def _execute_k8s_pod_create_task(self, task):
        return self.k8sKindExecutor._execute_k8s_pod_create_task(task)

    async def _execute_k8s_pod_status_task(self, task):
        return self.k8sKindExecutor._execute_k8s_pod_status_task(task)

    async def _execute_k8s_pod_delete_task(self, task):
        return self.k8sKindExecutor._execute_k8s_pod_delete_task(task)

    async def _execute_kind_cluster_create_task(self, task):
        return self.k8sKindExecutor._execute_kind_cluster_create_task(task)

    async def _execute_kind_cluster_delete_task(self, task):
        return self.k8sKindExecutor._execute_kind_cluster_delete_task(task)

    async def _execute_kind_cluster_list_task(self, task):
        return self.k8sKindExecutor._execute_kind_cluster_list_task(task)

    async def _execute_kind_cluster_get_nodes_task(self, task):
        return self.k8sKindExecutor._execute_kind_cluster_get_nodes_task(task)

    async def _execute_kind_load_image_task(self, task):
        return self.k8sKindExecutor._execute_kind_load_image_task(task)

    async def _execute_ollama_pull_task(self, task):
        return self.miscExecutor._execute_ollama_pull_task(task)

    async def _execute_ollama_run_task(self, task):
        return self.miscExecutor._execute_ollama_run_task(task)
