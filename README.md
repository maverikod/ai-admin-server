...
<!-- mcp-line-sweep-reversible-test-20260407 -->
...
Согласовано с `docs/PROJECT_RULES.md` (LAYOUT-*) и `docs/agents/project_overlay.md`.

```
vast_srv/
├── ai_admin/                 # Продакшен-пакет (PACKAGE_ROOT): команды, core, security, очередь
├── commands/                 # Дубли/наследие команд у корня (см. project_overlay) — по задаче
├── config/                   # Рабочие примеры конфигурации (копия из *.example.json)
├── configs/                  # Доп. не-секретные образцы под LAYOUT-04 (см. configs/README.md)
├── tests/                    # Pytest (LAYOUT-02)
├── scripts/                  # Операционные и не-pytest прогоны (LAYOUT-07)
├── logs/                     # Логи рантайма (LAYOUT-03)
├── docs/                     # Документация, планы
│   ├── agents/               # Контекст агентов, общие правила, overlay
│   ├── ai_reports/           # Черновики отчётов ИИ (LAYOUT-06)
│   └── PROJECT_RULES.md      # Канон: CR-*, LAYOUT-*, NAME-*
├── .cursor/
│   ├── rules/                # priority.mdc, project_canonical.mdc
│   └── agents/               # Роли иерархии (orchestrator, tactical, coder, …)
├── projectid                 # Идентификатор проекта (CR-003), если используется
├── docker/                   # Docker-артефакты
├── results/                  # Результаты прогонов / отчётов
├── requirements.txt
├── pyproject.toml
└── README.md
```

**Правила для ассистента в Cursor:** `.cursor/rules/project_canonical.mdc`, `.cursor/rules/priority.mdc`, полный профиль — `docs/PROJECT_RULES.md` (раздел 7 — Filled profile). Протокол сообщений между ролями: `docs/agents/formal_interaction_protocol.md`.
