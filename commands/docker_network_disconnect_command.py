...
class DockerNetworkDisconnectCommand(BaseUnifiedCommand):
        ...
        **kwargs,
        ...
        try:
            ...
            if result.returncode != 0:
                raise InternalError(f"Failed to disconnect container from network: {result.stderr}")
            ...
            raise InternalError(f"Docker network disconnect command timed out: {str(e)}")
        except InternalError as e:
            raise InternalError(f"Docker network disconnect failed: {str(e)}")
