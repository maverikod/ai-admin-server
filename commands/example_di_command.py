...
from ai_admin.dependency_injection import get_config, get_settings_manager, get_security_integration

class ExampleDICommand(BaseUnifiedCommand):
    """Example command that demonstrates dependency injection."""
    ...
    async def _show_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "dependencies": {
                ...
                "security_integration": "loaded" if self.security_integration else "not loaded",
            },
    ...
    async def _test_dependencies(self) -> Dict[str, Any]:
        ...
        try:
            if self.settings_manager:
                results["settings_manager"] = {"status": "ok", "type": type(self.settings_manager).__name__}
            else:
                results["settings_manager"] = {"status": "error", "message": "Settings manager not loaded"}
        except ConfigurationError as e:
        ...
        try:
            if self.security_integration:
                results["security_integration"] = {"status": "ok", "type": type(self.security_integration).__name__}
            else:
                results["security_integration"] = {"status": "error", "message": "Security integration not loaded"}
