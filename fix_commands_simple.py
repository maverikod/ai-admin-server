#!/usr/bin/env python3
"""Simple script to fix undefined variable errors in command files.
...
def fix_execute_method(content: str) -> str:
    ...
    execute_pattern = r'async def execute\(self, \*\*kwargs: Any\) -> CommandResult:'
    
    if not re.search(execute_pattern, content):
        return content  # No need to fix if already correct
    
    # Extract parameters from docstring
    params = extract_parameters_from_docstring(content)
    
    # Find the return statement and fix it
    return_pattern = r'(\s+)return await super\(\)\.execute\(\s*\*\*kwargs,\s*\)'
    ...
    def replace_return(match):
        ...
        param_extractions = []
        for param in params:
            param_extractions.append(f'{indent}        {param} = kwargs.get("{param}")')
        
        param_extraction_code = '\n'.join(param_extractions)
        if param_extraction_code:
            param_extraction_code = f'\n{indent}        # Extract parameters from kwargs\n{param_extraction_code}\n'
        
        # Create the new return statement
        if params:
            param_args = [f'{param}={param}' for param in params]
            return_statement = f'{indent}return await super().execute(\n'
            for param_arg in param_args:
                return_statement += f'{indent}            {param_arg},\n'
            return_statement += f'{indent}            **kwargs,\n{indent})'
        else:
            return_statement = f'{indent}return await super().execute(**kwargs)'
        
        return param_extraction_code + return_statement
    
    return re.sub(return_pattern, replace_return, content, flags=re.MULTILINE | re.DOTALL)
