...
from .queue_helpers import get_docker_commands
...
def add_queue_logic_to_command(file_path: str) -> bool:
    ...
    if 'add_build_task' in content or 'add_pull_task' in content or 'add_push_task' in content:
        print(f"  ✅ {file_path} already has queue logic")
        return True
    
    # Find the main execution method (usually _run_container, _build_image, etc.)
    execution_methods = [
        '_run_container', '_build_image', '_pull_image', '_push_image', 
        '_stop_container', '_start_container', '_restart_container',
        '_remove_container', '_inspect_object', '_list_containers',
        '_list_images', '_tag_image', '_login_registry', '_copy_files',
        '_manage_volume', '_manage_network', '_search_images'
    ]
    ...
    method_pattern = rf'async def {execution_method}\(\s*self,([^)]*)\) -> Dict\[str, Any\]:'
